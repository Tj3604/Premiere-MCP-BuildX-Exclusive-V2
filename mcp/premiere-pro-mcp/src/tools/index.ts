...
export class PremiereProTools {
    ...
    return [
      ...
      // Keyframes
      {
        ...
        inputSchema: z.object({
          ...
          value: z.union([z.number(), z.array(z.number())]).describe('The value to set at this keyframe. Use an array for multi-dimensional parameters — Position and Anchor Point take [x, y] in NORMALIZED coordinates where [0.5, 0.5] is frame centre.')
        })
      },
      {
        name: 'set_param_value',
        description: 'Sets a clip parameter to a static value with no keyframes (e.g. reposition or scale a logo). Use this instead of add_keyframe when the value should not animate.',
        inputSchema: z.object({
          clipId: z.string().describe('The ID of the clip'),
          componentName: z.string().describe('The display name of the component (e.g., "Motion", "Opacity")'),
          paramName: z.string().describe('The display name of the parameter (e.g., "Position", "Scale")'),
          value: z.union([z.number(), z.array(z.number())]).describe('The value. Use an array for multi-dimensional parameters — Position and Anchor Point take [x, y] in NORMALIZED coordinates where [0.5, 0.5] is frame centre.')
        })
      },
      {
  ...
  private async exportSequence(sequenceId: string, outputPath: string, presetPath?: string, format?: string, quality?: string, resolution?: string, range: 'entire' | 'inout' | 'workarea' = 'entire'): Promise<any> {
    ...
    try {
      ...
      const result = await this.bridge.renderSequence(sequenceId, outputPath, presetPath, range);
    ...
    const script = `
      try {
        ...
        if (!sequence) {
          ...
          // Premiere's marker collection is not index-addressable, and
          // deleteMarker() takes the marker OBJECT — passing an index throws
          // "Illegal Parameter type". Walk the linked list instead.
          var deleted = false;
          var target = ${JSON.stringify(markerId)};
          var marker = sequence.markers.getFirstMarker();
          while (marker) {
            var nextMarker = sequence.markers.getNextMarker(marker);
            if (String(marker.guid) === target) {
  ...
  private async addKeyframe(clipId: string, componentName: string, paramName: string, time: number, value: number | number[]): Promise<any> {
    const script = `
      try {
        ...
        param.addKey(${time});
        param.setValueAtKey(${time}, ${JSON.stringify(value)}, true);
        return JSON.stringify({
          success: true,
  ...
  private async setParamValue(clipId: string, componentName: string, paramName: string, value: number | number[]): Promise<any> {
    const script = `
      try {
        var info = __findClip(${JSON.stringify(clipId)});
        if (!info) return JSON.stringify({ success: false, error: "Clip not found" });
        ...
        if (!param) return JSON.stringify({ success: false, error: "Parameter " + ${JSON.stringify(paramName)} + " not found in component " + ${JSON.stringify(componentName)} });
        // Drop any existing keyframes so the value is genuinely static.
        if (param.isTimeVarying()) { param.setTimeVarying(false); }
        param.setValue(${JSON.stringify(value)}, true);
        return JSON.stringify({
          success: true,
