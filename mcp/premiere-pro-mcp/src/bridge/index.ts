...
export class PremiereProBridge implements PremiereProTransport {
  ...
  private async detectPremiereProInstallation(): Promise<void> {
    // Discover any installed year-versioned Premiere Pro rather than hardcoding years,
    // so new releases (2025, 2026, ...) are detected without a code change.
    const commonPaths: string[] = [];

    try {
      const appDirs = await fs.readdir('/Applications');
      for (const dir of appDirs.sort().reverse()) {
        if (dir.startsWith('Adobe Premiere Pro')) {
          commonPaths.push(`/Applications/${dir}/${dir}.app`);
        }
      }
    } catch (error) {
      // /Applications unreadable (non-macOS or sandboxed) — fall through to Windows paths
    }
  ...
  async renderSequence(sequenceId: string, outputPath: string, presetPath: string, range: 'entire' | 'inout' | 'workarea' = 'entire'): Promise<any> {
    ...
    const script = `
      try {
        ...
        } else if (requested === "inout") {
          range = (typeof app.encoder.ENCODE_IN_TO_OUT !== "undefined") ? app.encoder.ENCODE_IN_TO_OUT : 1;
        } else {
          range = (typeof app.encoder.ENCODE_ENTIRE !== "undefined") ? app.encoder.ENCODE_ENTIRE : 0;
        }
